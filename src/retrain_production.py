...
def retrain_production_models(data_path: str = "data/raw"):
    ...
    if cv_report_path.exists():
        print(
            f"[INFO] Found Nested CV report at {cv_report_path}. Loading HPO parameters..."
        )
        ...
        print(
            "[INFO] No Nested CV HPO parameters found. Using robust scientific fallbacks..."
        )
    ...
    if "barcode" in df.columns:
        ...
        n_rows = len(df)
        print(
            f"[BARCODE AUDIT] {n_rows} training rows, {n_unique_barcodes} unique molecular barcodes"
        )
    ...
    train_smiles, test_smiles = split_smiles_globally(
        df["smiles"].unique(), test_size=0.2, random_state=42
    )
    ...
    # Save train-only lookup to prevent predictor from returning experimental test values
    train_lookup = {}
    ...
    train_lookup_path = Path("data/processed/db_lookup_train.json")
    with open(train_lookup_path, "w") as f:
        json.dump(train_lookup, f, indent=2, sort_keys=True)
    print(
        f"[INFO] Saved train-only lookup ({len(train_lookup)} molecules) to {train_lookup_path}"
    )
    ...
    with open("models/scaler.pkl", "wb") as f:
        pickle.dump(pipeline, f)
    print(
        "[SUCCESS] Saved production scaler pipelines to models/scaler.pkl and models/precise/"
    )
    ...
    for st in SUBTYPES:
        ...
        if len(y_tr) < 50:
            print(
                f"  [SKIP] Insufficient training data for {st} ({len(y_tr)} samples)."
            )
            continue
        ...
        shutil.copy(
            xgb_path, Path("models") / f"xgboost_precise_{st.lower()}_model.pkl"
        )
        ...
        # Save RF model
        with open(
            Path("models/precise") / f"rf_precise_{st.lower()}_model.pkl", "wb"
        ) as f:
            pickle.dump(rf_model, f)
        with open(Path("models") / f"rf_{st.lower()}_model.pkl", "wb") as f:
            pickle.dump(rf_model, f)
        print(
            f"  [SUCCESS] Random Forest model saved to models/rf_{st.lower()}_model.pkl"
        )
