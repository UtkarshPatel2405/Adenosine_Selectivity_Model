...
def load_evaluation_tables(
    base: str = "outputs/validoutput/standard",
        ...
        [
            {
                ...
                "Conformal Coverage (90%)": f"{overall.get('conformal_coverage_90', 0.0) * 100:.1f}%"
                if overall.get("conformal_coverage_90") is not None
                else "N/A",
            }
        ]
    )
        ...
        rows.append(
            {
                ...
                "Conformal Coverage (90%)": f"{m.get('conformal_coverage_90', 0.0) * 100:.1f}%"
                if m.get("conformal_coverage_90") is not None
                else "N/A",
                "n_test": m.get("n_test"),
            }
        )
...
def load_fingerprint_comparison(
    base: str = "outputs/validoutput/standard",
) -> pd.DataFrame:
    infix = _get_mode_infix(base)
...
def _examples_to_df(path: str) -> pd.DataFrame:
    ...
    for item in data:
        ...
        preds_str = (
            ", ".join([f"{k}: {v:.2f}" for k, v in preds.items()]) if preds else "N/A"
        )
