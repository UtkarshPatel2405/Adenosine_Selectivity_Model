...
def predict_batch(
    ...
    novel_mask = (not res_df["in_database"]) & (res_df["canonical_smiles"].notna())
    if novel_mask.any():
        ...
        for j, s in enumerate(to_predict):
            if status_area:
                status_area.text(
                    f"Phase 2/3: Featurizing novel molecules ({j + 1}/{total_predict})"
                )
        ...
        for k, st_name in enumerate(SUBTYPES):
            ...
            if type(ensemble).__name__ == "CrossConformalRegressor":
                ...
                if y_pis.ndim == 3:
                    ...
                    lower = y_pis[:, 0]
                    upper = y_pis[:, 1]
                res_df.loc[novel_mask, f"{st_name}_uncertainty"] = (
                    upper - lower
                ) / 3.29
                ...
                res_df.loc[novel_mask, f"{st_name}_uncertainty"] = member_preds.std(
                    axis=0, ddof=0
                )
            else:
    ...
    if valid_mask.any():
        if status_area:
            status_area.text("Finalizing result dashboard...")
        res_df.loc[valid_mask, "best_target"] = res_df.loc[valid_mask, SUBTYPES].idxmax(
            axis=1
        )
