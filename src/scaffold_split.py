...
def _murcko_scaffold_smiles(smiles: str) -> str:
    ...
    try:
        scaf_smiles = MurckoScaffold.MurckoScaffoldSmiles(
...
def scaffold_split(
    ...
    train_df = (
        df_copy.iloc[train_indices].drop(columns=["_scaffold"]).reset_index(drop=True)
    )
    test_df = (
        df_copy.iloc[test_indices].drop(columns=["_scaffold"]).reset_index(drop=True)
    )

    return train_df, test_df


def split_smiles_globally(
    smiles_list: list, test_size: float = 0.2, random_state: int = 42
) -> tuple[set, set]:
    """
