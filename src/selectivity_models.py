...
def build_selectivity_models(data_path: str = "data/raw", min_paired: int = 50):
    ...
    for subA, subB in pairs:
        ...
        if n_paired < min_paired:
            print(f"  [SKIP] Too few paired compounds (< {min_paired}). Skipping direct model.")
            continue
...
def predict_direct_selectivity(smiles: str, subA: str, subB: str) -> float | None:
    ...
    if not model_path.exists() or not pipeline_path.exists():
        ...
        rev_model_path = Path(f"models/selectivity/xgb_selectivity_{reverse_pair_name}_model.pkl")
        rev_pipeline_path = Path(f"models/selectivity/xgb_selectivity_{reverse_pair_name}_pipeline.pkl")
