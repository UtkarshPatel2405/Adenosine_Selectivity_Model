    ...
    for subtype, filepath in gpcrdb_files.items():
        ...
        for col in df.columns:
            ...
            if "smiles" in col_lower:
                smiles_col = col
            if (
                "p-value" in col_lower
                or "pchembl" in col_lower
                or "p_value" in col_lower
            ):
                pval_col = col

        if smiles_col is None or pval_col is None:
            print(
                f"[WARNING] Could not find SMILES/pValue columns in {filepath}. Columns: {list(df.columns)}"
            )
            continue
        ...
        print(
            f"  {subtype}: {n_total} total, {n_novel} novel, {n_seen} seen (filtered out)"
        )
...
def run_external_validation(
    novel_df: pd.DataFrame = None, mode: str = "precise"
) -> dict:
    ...
    if len(subtypes_with_truth) >= 2:
        multi_target = result_df.dropna(
            subset=[f"{st}_true" for st in subtypes_with_truth], thresh=2
        )
        correct = 0
        total = 0
        ...
        for _, row in multi_target.iterrows():
            true_vals = {
                st: row[f"{st}_true"]
                for st in subtypes_with_truth
                if pd.notna(row.get(f"{st}_true"))
            }
            pred_vals = {
                st: row[f"{st}_pred"]
                for st in true_vals.keys()
                if pd.notna(row.get(f"{st}_pred"))
            }
    ...
    # Save report
    report = {
        "n_novel_molecules": len(novel_df),
        "n_successful_predictions": len(results),
        "n_errors": errors,
        "per_subtype_metrics": metrics,
    }

    out_dir = Path("outputs/external_validation")
    out_dir.mkdir(parents=True, exist_ok=True)
    with open(out_dir / "external_validation_report.json", "w") as f:
        json.dump(report, f, indent=2)
    print(
        f"\n[SUCCESS] External validation report saved to {out_dir / 'external_validation_report.json'}"
    )
