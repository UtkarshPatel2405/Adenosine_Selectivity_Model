...
class AdenosinePharmacophoreAnalyzer:
    ...
    def analyze_molecule(cls, smiles: str) -> Dict[str, Any]:
        ...
        hbd_pat = Chem.MolFromSmarts(
            cls.CORE_FEATURES["Conserved H-Bond Donor (Asn253/Glu169 interaction)"]
        )
        ...
        for st, features in cls.SUBTYPE_SELECTIVITY_FEATURES.items():
            ...
            # Normalize to 0-100%
            subtype_specific_score = (
                (st_matched / total_st_features) if total_st_features > 0 else 0
            )
