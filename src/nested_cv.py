...
def run_fold(
    ...
    print(
        f"[INFO] Dataset split: Train={len(train_df)} compounds, Test={len(test_df)} compounds"
    )

    # 3. Build features globally on this outer train/test split
    X_train, X_test, pipeline = build_feature_matrix(
...
def merge_results():
    ...
    if not out_dir.exists():
        print(
            "[ERROR] outputs/nested_cv directory does not exist! No results to merge."
        )
        return
    ...
    for subtype, folds_data in results_by_subtype.items():
        ...
        for param_key in all_params[0].keys():
            ...
            if isinstance(param_vals[0], (int, float)):
                median_params[param_key] = (
                    float(np.median(param_vals))
                    if isinstance(param_vals[0], float)
                    else int(np.round(np.median(param_vals)))
                )
            else:
    ...
    with open(md_file, "w") as f:
        ...
        f.write(
            "This report aggregates results from the chunk-and-merge Nested Cross-Validation runs using a scaffold split.\n\n"
        )
        ...
        for st in SUBTYPES:
            if st in merged_summary:
                ...
                f.write(
                    f"| **{st}** | {nf}/5 | {m['r2_mean']:.3f} ± {m['r2_std']:.3f} | {m['mae_mean']:.3f} ± {m['mae_std']:.3f} | {m['rmse_mean']:.3f} ± {m['rmse_std']:.3f} |\n"
                )
            else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Chunk-and-Merge Nested Cross-Validation with Optuna HPO"
    )
    parser.add_argument("--subtype", choices=SUBTYPES, help="Target subtype to run")
    parser.add_argument(
        "--fold", type=int, choices=[1, 2, 3, 4, 5], help="Outer fold index (1 to 5)"
    )
    parser.add_argument(
        "--trials",
        type=int,
        default=20,
        help="Number of Optuna HPO trials in the inner loop",
    )
    parser.add_argument(
        "--merge",
        action="store_true",
        help="Merge all completed fold results and write report",
    )
