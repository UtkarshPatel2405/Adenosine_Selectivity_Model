...
def run_shap_analysis(subtype: str = "A2A", mode: str = "precise"):
    ...
    if not model_path.exists():
        print(
            f"[ERROR] No model found for {subtype}. Complete production retraining first."
        )
        return None
    ...
    test_df_full = df[df["canonical_smiles"].isin(splits["test"])].reset_index(
        drop=True
    )
    ...
    test_df_full[subtype_mask].reset_index(drop=True)
    ...
    plt.title(
        f"SHAP Global Feature Importance (Beeswarm): {subtype}",
        fontsize=13,
        fontweight="bold",
    )
    plt.tight_layout()
    ...
    plt.title(
        f"SHAP Global Feature Importance (Bar): {subtype}",
        fontsize=13,
        fontweight="bold",
    )
    plt.tight_layout()
    ...
    for rank, idx in enumerate(sorted_indices[:10], 1):
        ...
        top_features.append(
            {"rank": rank, "feature": name, "mean_abs_shap": float(val)}
        )
    ...
    matching_expected = [
        f for f in expected_top if any(f in item["feature"] for item in top_features)
    ]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--all", action="store_true", help="Run SHAP for ALL 4 subtypes"
    )
