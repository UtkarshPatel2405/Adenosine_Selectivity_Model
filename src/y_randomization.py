...
def run_y_randomization(subtype: str = "A2A", n_iterations: int = 20, data_path: str = "data/raw/AR_all_unique_parents_with_smiles.csv"):
    print(f"\n" + "="*60)
    ...
    if len(df_st) < 50:
        print(f"[ERROR] Insufficient data for {subtype}")
        return
        
    df_st = df_st.rename(columns={"canonical_smiles": "smiles"})
    
    # 2. Perform base scaffold split (80-20)
    train_df, test_df = scaffold_split(df_st, test_size=0.2, random_state=42)
    ...
    for i in range(n_iterations):
        ...
        if (i+1) % 5 == 0:
            print(f"  Completed iteration {i+1}/{n_iterations} | R² = {shuffled_r2:.4f}")
            
    mean_shuffled_r2 = float(np.mean(shuffled_r2s))
    std_shuffled_r2 = float(np.std(shuffled_r2s))
    
    print(f"\n[RESULTS] Y-Randomization Summary:")
    ...
    if mean_shuffled_r2 > 0.10:
        print("[WARNING] Shuffled R² > 0.10! Possible target leakage or severe overfitting to background features.")
        leakage_flag = True
    else:
        print("[SUCCESS] Shuffled R² is near-zero/negative. The model relies on actual chemical structures rather than noise.")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--subtype", default="A2A", help="Subtype to validate")
